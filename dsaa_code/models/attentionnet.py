...
class Attn_Net(nn.Module):
    def __init__(self, im_size, num_classes, attention=None, normalize_attn=True, init='kaimingNormal'):
        ...
        self.dense = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=(im_size//32),  padding=0, bias=True)
        ...
        if any(self.attention):
            ...
            self.attns = nn.ModuleList([LinearAttentionBlock(in_features=512, normalize_attn=normalize_attn) for i in self.positions])
    ...
    def forward(self, x):
        ...
        g = self.dense(l6)  # batch_sizex512x1x1

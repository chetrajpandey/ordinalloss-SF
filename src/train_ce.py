...
# from custom_loss import FocalLoss, OrdinalFocalLoss, OrdinalCrossEntropyLoss
...
# with open("/home/users/cpandey1/sweep.yaml", "r") as config_file:
#     configure = yaml.safe_load(config_file)
# Load configuration from YAML file
# with open("/home/users/cpandey1/sweep.yaml", "r") as config_file:
#     config = yaml.safe_load(config_file)

wandb.init(project='tune_ce', save_code=True)
config = wandb.config
...
# parser.add_argument("--im_size", type=int, default=512, help="image_size")
# parser.add_argument("--attn_config", nargs='+', type=str_to_bool, default=[True, True, True, True, True, True], help="list of booleans (True and/or False) with six items, each indicating activation/deactivation of attention layers")
parser.add_argument("--saving_dir", type=str, default='CE', help="Enter model name for saving directory for corresponding model")
# parser.add_argument("--lon", type=str, default='-90_to_90', help="ar patch coordinates. Use case: --lon 30_to_30")
...
# parser.add_argument("--class_weights", type=int, default=config.class_weights, help="imbalance parameter")
...
def dataloading():
    ...
    csv_file_train = f'/scratch/users/cpandey1/ar_data_labels/final_ecml/final/train_new.csv'
    csv_file_val = f'/scratch/users/cpandey1/ar_data_labels/final_ecml/final/val_new.csv'
    csv_file_test = f'/scratch/users/cpandey1/ar_data_labels/final_ecml/final/test_new.csv'
...
def train():
    ...
    # print("Model Attention Configuration: ", attention_config)
    ...
    for epoch in range(1,num_epochs):
        ...
        print(f'Training--> loss: {train_loss:.2f}, TSS: {train_tss:.4f}, HSS2: {train_hss:.4f} | Val--> loss: {val_loss:.4f}, TSS: {val_tss:.4f} | HSS2: {val_hss:.4f} ')
